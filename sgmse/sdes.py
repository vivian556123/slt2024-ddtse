...
class SDE(abc.ABC):
    ...
    def discretize(self, x, t, *args):
        ...
        #print("discretize")
        dt = 1 / self.N
        drift, diffusion = self.sde(x, t, args[0])
        f = drift * dt
    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def rsde_parts(self, x, t, *args):
                #print("rsde_parts",args[0])
                sde_drift, sde_diffusion = sde_fn(x, t, args[0])
                score = score_model.sample_forward(x, t, *args)
                score_drift = -sde_diffusion[:, None, None, None]**2 * score * (0.5 if self.probability_flow else 1.)
                #print("sde_drift", sde_drift.shape)
            ...
            def discretize(self, x, t, *args):
                #print("rsde discretize")
                """Create discretized iteration rules for the reverse diffusion sampler."""
                f, G = discretize_fn(x, t, *args)
                rev_f = f - G[:, None, None, None] ** 2 * score_model.sample_forward(x, t, *args) * (0.5 if self.probability_flow else 1.)
...
class OUVESDE(SDE):
    ...
    def sde(self, x, t, y):
        #print("y", type(y),y)
        drift = self.theta * (y - x) #Calculate f(x,t)
        ...
        diffusion = sigma * np.sqrt(2 * self.logsig) #Calculate g(t)
        #print("self.sde", sigma, diffusion)
...
class ConditionalSDE(abc.ABC):
    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def sde(self, x, t, *args):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                rsde_parts = self.rsde_parts(x, t, args[0])
                total_drift, diffusion = rsde_parts["total_drift"], rsde_parts["diffusion"]
                return total_drift, diffusion
...
@SDERegistry.register("c_ouve")
class ConditionalOUVESDE(SDE):
    @staticmethod
    def add_argparse_args(parser):
        parser.add_argument("--sde-n", type=int, default=1000, help="The number of timesteps in the SDE discretization. 30 by default")
        parser.add_argument("--theta", type=float, default=1.5, help="The constant stiffness of the Ornstein-Uhlenbeck process. 1.5 by default.")
